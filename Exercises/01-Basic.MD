...
or to check FS uase of each FS mount and diplay 80% exceeded FS mounts
```
import psutil
def fs_usage(path):
    data = psutil.disk_usage(path)
    usage = data.used *100/data.total
    if usage > 80:
        print(f"FS usgae of {path} is {usage}% - exceeded threshold of 80%")

mounts = [partition.mountpoint for partition in psutil.disk_partitions()]
...
```
### read log file log.txt and search for "error" and write error lines to error.txt file
```
input_file = "C:\\Users\\MakkenaSuresh\\Desktop\\test\\FEB22_Python\\logs.txt"
output_file = "C:\\Users\\MakkenaSuresh\\Desktop\\test\\FEB22_Python\\error.txt"
with open(input_file, "r") as infile, open(output_file, "w") as outfile:
...
```
example output of servers:
```
[{'NAME ': 'erw', 'VALUE': '90'}, {'NAME ': 'rwe', 'VALUE': '3'}, {'NAME ': 'wre', 'VALUE': '879'}, {'NAME ': 'rwewre', 'VALUE': '90'}, {'NAME ': 'ew', 'VALUE': '98'}, {'NAME ': 'rwe', 'VALUE': '78'}, {'NAME ': 'wr', 'VALUE': '87'}, {'NAME ': 'gd', 'VALUE': '56'}, {'NAME ': 'g', 'VALUE': '879'}, {'NAME ': 'vb', 'VALUE': '79'}]
```
